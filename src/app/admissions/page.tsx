...
const requiredDocs = [
  ...
  { name: "Prior Class Marksheet", size: "PDF/JPEG, Max 5MB", note: "Official certified academic record" },
...
export default function AdmissionsPage() {
  ...
  const [marksheetName, setMarksheetName] = useState<string | null>(null);
  ...
  const fileInputMarksheetRef = useRef<HTMLInputElement>(null);
  ...
  const stepsFields: (keyof AdmissionsFormValues)[][] = [
    ["studentName", "dob", "gender", "religion", "bloodGroup", "studentPhoto"],
    ["fatherName", "motherName", "parentPhone", "altPhone", "parentEmail", "emergencyContact", "password", "confirmPassword"],
    ["prevSchool", "prevClass", "grade", "marksheet"],
    ["aadharNumber", "aadharImage", "address", "city", "state", "pinCode", "notes"]
  ];
  ...
  const handleMarksheetUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      alert("Marksheet size must be less than 5MB");
      return;
    }
    ...
    try {
      const ext = file.name.split(".").pop();
      const fileName = `marksheets/${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`;
      ...
      setValue("marksheet", data.publicUrl, { shouldValidate: true });
    } catch (err) {
      console.error("Marksheet upload failed:", err);
      alert("Failed to upload marksheet. Please check your bucket settings and try again.");
      setMarksheetName(null);
      setValue("marksheet", "", { shouldValidate: true });
    } finally {
        ...
        <div id="admission-form-container" className="mx-auto max-w-4xl px-4 mt-24 sm:px-6">
          {admissionsOpen === null ? (
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/5 pb-6">
              <div>
                <span className="text-xs font-semibold text-gold-400 uppercase tracking-widest">Step {currentStep + 1} of 4</span>
                ...
                >
                    ...
                    <div className="space-y-5">
                      ...
                      <div className="grid gap-5 sm:grid-cols-3">
                        ...
                        <label className="block">
                          ...
                          <input
                            type="text"
                            placeholder="e.g. Christian, Hindu"
                            {...register("religion")}
                        ...
                        <label className="block">
                          <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex justify-between">
                            ...
                            {errors.bloodGroup && (
                              <span className="text-[10px] text-red-400 font-medium normal-case">{errors.bloodGroup.message as string}</span>
                            )}
                          </span>
                          <select
                            {...register("bloodGroup")}
                            className={cn(darkInputClass, errors.bloodGroup && "border-red-500/50 focus:border-red-500 focus:ring-red-500/20")}
                          >
                            <option value="" className="text-slate-900 bg-white">Select group</option>
                            <option value="A+" className="text-slate-900 bg-white">A+</option>
                            <option value="A-" className="text-slate-900 bg-white">A-</option>
                            <option value="B+" className="text-slate-900 bg-white">B+</option>
                            <option value="B-" className="text-slate-900 bg-white">B-</option>
                            <option value="AB+" className="text-slate-900 bg-white">AB+</option>
                            <option value="AB-" className="text-slate-900 bg-white">AB-</option>
                            <option value="O+" className="text-slate-900 bg-white">O+</option>
                            <option value="O-" className="text-slate-900 bg-white">O-</option>
                            <option value="Unknown" className="text-slate-900 bg-white">Unknown</option>
                          </select>
                        </label>
                      </div>
                      ...
                      <div className="grid gap-5 sm:grid-cols-2">
                        <label className="block">
                          ...
                          >
                            ...
                            <option value="Class 12" className="text-slate-900 bg-white">Class 12</option>
                          </select>
                        </label>
                      </div>

                      {/* Marksheet Upload Dropzone */}
                      <div>
                        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex justify-between">
                          <span>Upload Previous Marksheet *</span>
                          {errors.marksheet && (
                            <span className="text-[10px] text-red-400 font-medium normal-case">{errors.marksheet.message as string}</span>
                          )}
                        </span>
                        
                        <div 
                          onClick={() => !uploadingMarksheet && fileInputMarksheetRef.current?.click()}
                          className={cn(
                            "mt-2 border border-dashed rounded-xl p-6 text-center transition",
                            uploadingMarksheet ? "cursor-wait opacity-70" : "cursor-pointer hover:bg-white/5",
                            marksheetName && !uploadingMarksheet ? "border-emerald-500/50 bg-emerald-500/5" : "border-white/10 bg-slate-900/60"
                          )}
                        >
                          <input
                            type="file"
                            accept="application/pdf,image/*"
                            ref={fileInputMarksheetRef}
                          ...
                          {uploadingMarksheet ? (
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <p className="text-xs font-semibold text-blue-400 animate-pulse">Uploading marksheet to cloud…</p>
                            </div>
                          ) : marksheetName ? (
                            <div className="flex items-center justify-center gap-3">
                              <CheckCircle className="h-6 w-6 text-emerald-400" />
                              <div className="text-left">
                                <p className="text-xs font-bold text-emerald-400">File uploaded successfully!</p>
                                <p className="text-[10px] text-slate-300 font-mono">{marksheetName}</p>
                              </div>
                            </div>
                          ) : (
                            <div className="space-y-2">
                              <Upload className="h-8 w-8 text-blue-400 mx-auto" />
                              <p className="text-xs font-bold text-slate-300">Click to upload marksheet PDF/Image</p>
                              <p className="text-[10px] text-slate-400">Supports PDF, JPG formats up to 5MB</p>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  {currentStep === 3 && (
                    <div className="space-y-5">
                      ...
                      <label className="block">
                        <span className="text-xs font-semibold uppercase tracking-wider text-slate-400 flex justify-between">
                          <span>Additional Notes / Message</span>
                          {errors.notes && (
                            <span className="text-[10px] text-red-400 font-medium normal-case">{errors.notes.message as string}</span>
                          )}
                        </span>
                        <textarea
                          placeholder="Tell us about special skills, health restrictions, or concerns..."
                          rows={3}
                          {...register("notes")}
                          className={cn(darkInputClass, "resize-none", errors.notes && "border-red-500/50 focus:border-red-500 focus:ring-red-500/20")}
                        />
                      </label>
                    </div>
                  )}
            ...
            <AnimatePresence>
              {showSuccess && (
                ...
                >
                  ...
                  >
                    The admission profile has been successfully generated in our databases. A verification email has been sent to your parent address with registration details and evaluation schedules.
                  </motion.p>
                  ...
                  <motion.button
                    ...
                    className="mt-8 rounded-xl border border-white/10 bg-white/5 hover:bg-white/10 px-6 py-3 text-sm font-semibold text-slate-200 transition"
                  >
                    Fill Another Form
                  </motion.button>
                </motion.div>
