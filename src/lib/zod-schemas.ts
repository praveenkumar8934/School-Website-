...
export const admissionsSchema = z.object({
  ...
  parentPhone: z.string().trim().regex(/^\d{10}$/, "Enter a valid 10 digit mobile number"),
  altPhone: z.string().trim().regex(/^\d{10}$/, "Enter a valid alternate mobile number").or(z.string().length(0)),
  parentEmail: z.string().trim().email("Please enter a valid parent email address"),
  emergencyContact: z.string().trim().regex(/^\d{10}$/, "Enter a valid emergency contact number"),
